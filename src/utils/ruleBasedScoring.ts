...
export function ruleBasedAnalysis(ingredients: string[]): AnalysisResult {
  ...
  console.log('Harmful ingredients found:', harmfulFound.length);
  harmfulFound.forEach((h) => {
    console.log(`- ${h.name} (${h.severity}): ${h.description}`);
  });
  ...
  console.log('Healthy ingredients:', healthyCount);
  console.log('Processed ingredients:', processedCount);
...
function estimateNutrients(ingredients: string[]): AnalysisResult['nutrients'] {
  ...
  if (text.includes('salt') || text.includes('sodium')) {
    const sodiumPosition = ingredients.findIndex(
      (i) => i.toLowerCase().includes('salt') || i.toLowerCase().includes('sodium')
    );
...
function generateSummary(
  ...
  if (category === 'Healthy') {
    ...
    parts.push(
      'This product contains several concerning ingredients and should be consumed in moderation.'
    );
  }
  ...
  if (harmfulFound.length > 0) {
    ...
    if (highSeverity.length > 0) {
      parts.push(
        `Contains ${highSeverity.length} high-risk ingredient(s): ${highSeverity.map((h) => h.name).join(', ')}.`
      );
    }
  }
